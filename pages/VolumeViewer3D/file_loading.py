...
def collect_volume_files(data_dirs: List[str]) -> Tuple[List[str], List[str], Optional[Path]]:
    """
    Collect VTI and HDF5 files from all data directories.
        ...
        try:
            ...
            if d.exists() and d.is_dir():
                if data_dir is None:
                    data_dir = d
                dir_vti = sorted(
                    glob.glob(str(d / "*.vti")) + glob.glob(str(d / "*.VTI")),
                    key=natural_sort_key,
                )
                dir_hdf5 = sorted(
                    glob.glob(str(d / "*.h5")) + glob.glob(str(d / "*.H5"))
                    + glob.glob(str(d / "*.hdf5")) + glob.glob(str(d / "*.HDF5")),
                    key=natural_sort_key,
                )
                all_vti.extend(dir_vti)
                all_hdf5.extend(dir_hdf5)
        except Exception:
            continue
