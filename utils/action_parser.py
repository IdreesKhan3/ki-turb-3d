...
class ActionParser:
    ...
    def _get_system_prompt(self) -> str:
        ...
        return """You are an intelligent AI coding assistant. Analyze the user's prompt carefully and respond with appropriate actions.
...
- create_file: Create file (use "filepath", "content")
- rename_file: Rename/move file (use "filepath", "new_filepath")
- delete_file: Delete file (use "filepath"). System asks for confirmation.
- run_shell_command: Execute shell command (use "command")
- execute_code: Execute Python code (use "code")
- search_codebase: Search code (use "query")
- navigate: Navigate to page (use "target_page")
- load_data: Load data directory (use "path")
...
Analyze the user's prompt and choose the appropriate action(s). Use your intelligence to understand intent.
    ...
    def _format_context(self, context: Optional[Dict]) -> str:
        ...
        if context:
            ...
            if "current_page" in context:
                lines.append(f"Current page: {context['current_page']}")
            if "available_plots" in context:
                lines.append(f"Available plots: {', '.join(context['available_plots'])}")
            if "data_loaded" in context:
                lines.append(f"Data loaded: {context['data_loaded']}")
        ...
        try:
            ...
            # Discover pages directory
            pages_dir = project_root / "pages"
            if pages_dir.exists():
                pages = [f.name for f in pages_dir.glob("*.py") if f.name.startswith(("0", "1"))]
                if pages:
                    lines.append(f"Available pages: {', '.join(sorted(pages)[:20])}")
            ...
            if common_dirs:
                lines.append(f"Project directories: {', '.join(common_dirs)}")
        ...
        return "\n".join(lines) if lines else "Project context available - use your intelligence to discover structure"
