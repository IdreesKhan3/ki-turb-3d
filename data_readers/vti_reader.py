    ...
    try:
        with open(filepath, 'rb') as f:
            ...
            if nbyte == expected_nbyte_float64:
                ...
                dtype = np.float64
                print(f"Warning: nbyte mismatch. Expected {expected_nbyte_float64} or {expected_nbyte_float32}, got {nbyte}. Using float64.")
...
def compute_vorticity(velocity: np.ndarray, dx: float = 1.0, dy: float = 1.0, dz: float = 1.0) -> np.ndarray:
    ...
    # Central differences: ωx=∂uz/∂y-∂uy/∂z, ωy=∂ux/∂z-∂uz/∂x, ωz=∂uy/∂x-∂ux/∂y
    ux = velocity[:, :, :, 0]
    uy = velocity[:, :, :, 1]
    uz = velocity[:, :, :, 2]
