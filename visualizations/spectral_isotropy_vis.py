...
def create_ic_isotropy_figure(
    ...
    *,
    ...
    for idx, (sim_prefix, data) in enumerate(sim_items):
        ...
        valid = (k > 0) & np.isfinite(ic_mean) & (ic_mean > 1e-15)
        if not np.any(valid):
            continue
        k, ic_mean = k[valid], ic_mean[valid]
        ic_std = None
        if ic_std_raw is not None:
            arr = np.asarray(ic_std_raw)
        ...
        if style_override:
            color = style_override.get("color") or colors[idx % len(colors)]
            lw_use = style_override.get("width") or lw
            dash = style_override.get("dash") or "solid"
            ...
            color, lw_use, dash, marker, msize, override_on = resolve_line_style(
        ...
        trace_kwargs = dict(x=k, y=ic_mean, mode=mode, name=legend_name, line=dict(color=color, width=lw_use, dash=dash))
        ...
        if show_std_band and ic_std is not None and np.any(np.isfinite(ic_std)):
            try:
                rgb = hex_to_rgb(color) if isinstance(color, str) and color.startswith("#") else (65, 105, 225)
            except Exception:
                rgb = (65, 105, 225)
    ...
    *,
    ...
    for idx, (sim_prefix, data) in enumerate(sim_items):
        ...
        if style_override:
            ...
            color, lw_use, dash, _, _, _ = resolve_line_style(
                sim_prefix, idx, colors, ps,
        ...
        for i, curve in enumerate(curves_to_plot):
            ...
            if cs:
                c, w, d = cs.get("color") or color, cs.get("width") or lw_use, cs.get("dash") or dash
            else:
