...
class LLMProvider(ABC):
    ...
    def generate_stream_with_messages(self, messages: List[Dict[str, str]], **kwargs):
        """Generate streaming response using structured message history (yields chunks)"""
        pass
...
class GeminiLLM(LLMProvider):
    ...
    def generate(self, prompt: str, system_prompt: Optional[str] = None, **kwargs) -> str:
        ...
        try:
            ...
            # Note: response.text raises when no valid Part (e.g. finish_reason 12=RECITATION, safety block)
            content = None
            finish_reason = None
            try:
...
def get_llm_provider(
    ...
    Args:
        ...
        model_name: For Ollama: model tag (e.g. 'qwen2.5-coder:32b', 'mistral:7b').
                    For Gemini: model name. Overrides env vars.
