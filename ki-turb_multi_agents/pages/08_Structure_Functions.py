"""Structure functions / ESS page (Streamlit). Logic in pages/StructureFunctions/.

Reads .bin/.txt structure functions; plots S_p(r), ESS, and scaling anomalies.
Static export needs kaleido.
"""
...
def main():
    ...
    normalize_by_urms = st.sidebar.checkbox(
        "Normalize S_p by u_rms^p",
        value=bool(urms_s and float(urms_s) > 0.0),
        key="struct_norm_urms",
        help="Disabled by default when u_rms is missing/zero (avoids empty NaN plots).",
    )
    if normalize_by_urms and (urms_s is None or float(urms_s) <= 0.0):
        st.sidebar.warning("u_rms ≤ 0 — normalization skipped until a positive u_rms is available.")
        normalize_by_urms = False
