"""
Structure Functions — Plot styling (fonts, grids, per-sim overrides, reference curves).
"""
...
def _normalize_plot_name_local(plot_name: str) -> str:
    """Normalize plot name; handle Unicode (ξₚ) and special chars."""
...
def apply_plot_style(fig, ps: Dict[str, Any]):
    """Apply plot style to figure with Structure Functions defaults."""
    ...
    if any(k in ps for k in ["margin_left", "margin_right", "margin_top", "margin_bottom"]):
        r_val = ps.get("margin_right", 20)
        # Ensure minimum right margin to prevent clipping (legend or plot edge)
        r_val = max(int(r_val), 80)
        fig.update_layout(
            margin=dict(
...
def get_plot_style(plot_name: str) -> Dict[str, Any]:
    ...
    default.update({
        ...
        "she_leveque_color": "#000000",
        "experimental_b93_color": "#00BFC4",
    })
    if plot_name in ["Anomalies (ξₚ − p/3)", "ESS Inset"]:
        default.update({"x_axis_type": "linear", "y_axis_type": "linear"})
    else:
        default.update({"x_axis_type": "log", "y_axis_type": "log"})
    ...
    if "tick_color" not in plot_style or plot_style.get("tick_color") is None:
        merged["tick_color"] = None

    if "Dark" in current_theme and plot_name in ["Anomalies (ξₚ − p/3)", "ESS Inset"]:
        if merged.get("she_leveque_color") == "#000000":
            merged["she_leveque_color"] = "#569cd6"
        if merged.get("experimental_b93_color") == "#00BFC4":
            merged["experimental_b93_color"] = "#4ec9b0"
    return merged


STRUCTURE_PLOT_NAMES = ["S_p(r) vs r", "ESS (S_p vs S_3)", "ESS Inset", "Anomalies (ξₚ − p/3)"]


def plot_style_sidebar(data_dir: Path, sim_groups: Dict[str, Any], plot_names: List[str]):
    """Render plot style configuration in sidebar."""
    if "plot_styles" not in st.session_state:
        st.session_state.plot_styles = {}
    plot_styles = st.session_state.plot_styles
    # Ensure per_sim defaults exist for each structure plot
    for pn in STRUCTURE_PLOT_NAMES:
        if pn in plot_styles and sim_groups:
            ensure_per_sim_defaults(
                plot_styles[pn],
                sim_groups,
    ...
    selected_plot = st.sidebar.selectbox(
        "Select plot to configure", plot_names, key="structure_plot_selector"
    )
    ...
    with st.sidebar.expander("🎨 Plot Style (persistent)", expanded=False):
        ...
        ps["plot_bgcolor"] = st.color_picker("Plot background (inside axes)", color_to_hex(ps.get("plot_bgcolor") or "#FFFFFF"), key=f"{key_prefix}_plot_bgcolor")
        ps["paper_bgcolor"] = st.color_picker("Paper background (outside axes)", color_to_hex(ps.get("paper_bgcolor") or "#FFFFFF"), key=f"{key_prefix}_paper_bgcolor")
        st.markdown("---")
        st.markdown("**Ticks**")
        ...
        ps["grid_color"] = st.color_picker("Major grid color", color_to_hex(ps.get("grid_color") or "#B0B0B0"), key=f"{key_prefix}_grid_color")
        ...
        ps["minor_grid_color"] = st.color_picker("Minor grid color", color_to_hex(ps.get("minor_grid_color") or "#D0D0D0"), key=f"{key_prefix}_minor_grid_color")
        ...
        if selected_plot in ["Anomalies (ξₚ − p/3)", "ESS Inset"]:
            ps["she_leveque_color"] = st.color_picker("She–Leveque curve color", color_to_hex(ps.get("she_leveque_color") or "#000000"), key=f"{key_prefix}_she_leveque_color")
            ps["experimental_b93_color"] = st.color_picker("Experimental B93 color", color_to_hex(ps.get("experimental_b93_color") or "#00BFC4"), key=f"{key_prefix}_experimental_b93_color")
        if selected_plot == "ESS Inset":
            ps["zero_line_color"] = st.color_picker("Zero line (Kolmogorov reference) color", color_to_hex(ps.get("zero_line_color") or ps.get("axis_line_color") or "#000000"), key=f"{key_prefix}_zero_line_color")
            ps["zero_line_width"] = st.slider("Zero line width", 0.5, 3.0, float(ps.get("zero_line_width", ps.get("axis_line_width", 0.8))), key=f"{key_prefix}_zero_line_width")
        st.markdown("---")
        st.markdown("**Colors**")
        ...
        st.markdown("---")
        render_per_sim_style_ui(ps, sim_groups, style_key="per_sim_style_structure", key_prefix=f"{key_prefix}_sim", include_marker=True, show_enable_checkbox=True)
        st.markdown("---")
        if st.button("♻️ Reset Plot Style", key=f"{key_prefix}_reset"):
            ...
            widget_keys = [
                ...
                f"{key_prefix}_she_leveque_color", f"{key_prefix}_experimental_b93_color",
                f"{key_prefix}_zero_line_color", f"{key_prefix}_zero_line_width",
