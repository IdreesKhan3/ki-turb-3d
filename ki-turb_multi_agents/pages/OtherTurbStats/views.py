...
def render_custom_plot_section(
    ...
    # Use trace_key (data_source_x_col_y_col) for sim_groups so multiple traces from same
    # data_source (e.g. eps_spectral and eps_real) get distinct per-trace styling
    if st.session_state.get("custom_plot_traces"):
        sim_groups = {
            f"{t['data_source']}_{t['x_col']}_{t['y_col']}": []
            for t in st.session_state.custom_plot_traces
            if t.get("data_source") in all_dataframes and t.get("x_col") and t.get("y_col")
        }
    else:
        sim_groups = {key: [] for key in all_dataframes.keys()}
    if sim_groups:
        plot_style_sidebar(sim_groups)
