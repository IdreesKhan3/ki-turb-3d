...
from utils.file_detector import natural_sort_key, group_files_by_simulation, detect_simulation_files, expand_analysis_search_dirs
...
def load_files_and_groups() -> Optional[Tuple[Path, Dict[str, List[str]], Dict[str, List[str]]]]:
    ...
    if st.session_state.file_selection_mode == "directory":
        ...
        for data_dir_path in data_dirs:
            try:
                ...
                if not (d.exists() and d.is_dir()):
                    continue
                for search_dir in expand_analysis_search_dirs(d):
                    detected = detect_simulation_files(str(search_dir))
                    dir_spectrum = [str(f) for f in detected.get("spectrum", [])]
                    dir_norm = [str(f) for f in detected.get("norm_spectrum", [])]
                    if not dir_spectrum:
                        dir_spectrum = sorted(
                            glob.glob(str(search_dir / "spectrum*.dat")),
                            key=natural_sort_key,
                        )
                    if not dir_norm:
                        dir_norm = sorted(
                            glob.glob(str(search_dir / "norm*.dat")),
                            key=natural_sort_key,
                        )
        ...
        spectrum_files = list(dict.fromkeys(spectrum_files))
        norm_files = list(dict.fromkeys(norm_files))
