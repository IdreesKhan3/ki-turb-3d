...
def render_time_evolution(
    ...
    if n_files == 1:
        every_n = 1
        st.sidebar.caption("Show every Nth iteration: 1 (only one file)")
    else:
        every_n = st.sidebar.slider(
            "Show every Nth iteration curve",
            1,
            min(50, n_files),
            min(5, n_files - 1) if n_files > 1 else 1,
            key="energy_evol_every_n",
        )
    thin_idx = list(range(0, n_files, every_n))
    thin_files = [files[i] for i in thin_idx]
    thin_iters = [iters[i] for i in thin_idx]

    if len(thin_files) == 1:
        sel_pos = 0
        st.sidebar.caption("Highlight curve: 0 (only one thinned curve)")
    else:
        sel_pos = st.sidebar.slider(
            "Highlight curve (thinned index)",
            0,
            len(thin_files) - 1,
            len(thin_files) - 1,
