...
def main():
    ...
    default_title = f"Turbulence Analysis Report - {datetime.now().strftime('%Y-%m-%d')}"
    st.session_state.setdefault("report_title", default_title)
    report_title = st.sidebar.text_input("Title", key="report_title", help="Title for your report")
    
    st.session_state.setdefault("report_author", "")
    author = st.sidebar.text_input("Author", key="report_author", help="Your name or organization")
    ...
    st.session_state.setdefault("report_include_toc", True)
    include_toc = st.sidebar.checkbox("Include Table of Contents", key="report_include_toc")
