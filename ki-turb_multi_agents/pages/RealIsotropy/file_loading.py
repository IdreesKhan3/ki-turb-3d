...
def _find_eps_file(data_dir: Path) -> Optional[Path]:
    ...
    # Prefer files already indexed when an OpenLB manifest was loaded into the session.
    try:
        loaded = st.session_state.get("all_loaded_files") or {}
        for item in loaded.get("spectral_turb_stats", []):
            path = Path(str(item.get("full_path") or ""))
            name = path.name
            if path.is_file() and (
                name.startswith("eps_real_validation") or name.startswith("turbulence_validation")
            ):
                return path
    except Exception:
        pass
...
def _find_stress_file(data_dir: Path, eps_file: Path) -> Optional[Path]:
    ...
    search_dirs = [eps_file.parent, data_dir]
    try:
        loaded = st.session_state.get("all_loaded_files") or {}
        for item in loaded.get("reynolds_stress", []):
            path = Path(str(item.get("full_path") or ""))
            if path.is_file():
                return path
    except Exception:
    ...
    for search_dir in search_dirs:
        if tag:
            stress_with_tag = search_dir / f"reynolds_stress_validation{tag}.csv"
            if stress_with_tag.exists():
                return stress_with_tag
        exact_stress = search_dir / "reynolds_stress_validation.csv"
        if exact_stress.exists():
            return exact_stress
        matches = glob.glob(str(search_dir / "reynolds_stress_validation*.csv"))
        if matches:
            return Path(matches[0])
    return None
