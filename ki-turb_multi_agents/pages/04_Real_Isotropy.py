"""Real-space isotropy validation (Streamlit).

Reads eps_real_validation*/turbulence_validation* (and optional reynolds_stress_*),
computes b_ij / Lumley invariants, and plots energy fractions, Lumley triangle,
anisotropy components, and convergence. Static export needs kaleido.
"""
...
def main():
    ...
    stationary_iter = st.sidebar.number_input(
        "Stationarity iteration", value=50000.0, step=5000.0,
        key="real_iso_stationary_iter",
    )
    ...
    ma_win = st.sidebar.slider(
        "Moving average window (0=off)", 0, 500,
        value=st.session_state.get("real_iso_ma_win", default_ma_win),
        step=5,
        key="real_iso_ma_win",
    )
