...
ORGANIZATION: Content is sectionized by page. When adding a new page:
  ...
  6. Update role prompts in agents/langgraph/prompts.py
"""
...
PAGE_SCHEMA: Dict[str, Dict[str, Any]] = {
    # --- PAGE 00: agent chat entry (seven roles live in LangGraph) ---
    # --- PAGES 01-03: Overview, Theory, Multi-Method (no page-local compute/plot tools) ---
    "autonomous_lab": {
        "file_patterns": [],
        "compute_tool": None,
        "plot_tools": [],
...
INTENT_ROUTING: Dict[str, Dict[str, Any]] = {
    ...
    INTENT_SPECTRAL_ISOTROPY: {
        ...
        "intent_override": (
            ...
            "MULTI-SIM: steward ONCE for BOTH dirs, analyst ONCE with data_directories=[path1, path2]. Never split per dir. "
            "If user asked for styles/colors: include style_updates (per_sim_style_ic, palette, etc.) in the FIRST and ONLY plot_spectral_isotropy call. "
            "ONE plot only. STOP after producing it. Do NOT delegate plot_spectral_isotropy again.\n\n"
        ),
    },
    ...
    INTENT_COMPONENT_SPECTRA: {
        ...
        "intent_override": (
            ...
            "MULTI-SIM: steward ONCE for BOTH dirs, analyst ONCE with data_directories=[path1, path2]. Never split steward+analyst per dir. "
            "ONE plot only. STOP after producing the plot. Do not delegate to visualizer again.\n\n"
        ),
    },
    ...
    INTENT_ENERGY_SPECTRA: {
        ...
        "intent_override": (
            ...
            "MULTI-SIM: steward ONCE for BOTH dirs, analyst ONCE with data_directories=[path1, path2]. Never split per dir. "
            "If user asked for styles/colors: include style_updates (per_sim_style_raw, per_sim_style_norm, palette, etc.) in the FIRST and ONLY plot_spectrum call. "
        ...
        "intent_override_evolution": (
            ...
            "MULTI-SIM: steward ONCE for BOTH dirs, analyst ONCE with data_directories=[path1, path2]. Never split per dir. "
            "If user asked for styles/colors: include style_updates in the FIRST and ONLY plot_spectrum call. "
    ...
    INTENT_FLATNESS: {
        ...
        "intent_override": (
            ...
            "MULTI-SIM: steward ONCE for BOTH dirs, analyst ONCE with data_directories=[path1, path2]. Never split per dir. "
            "If user asked for plot: visualizer plot_flatness (include style_updates in that call). ONE plot only. STOP after producing it. "
            "If user asked for summary/table: visualizer get_flatness_summary (uses cached data from compute_flatness)."
        ),
    },
    ...
    INTENT_STRUCTURE_FUNCTIONS: {
        ...
        "intent_override": (
            ...
            "When Context has 'Computed structure functions' or data_reference=current_structure_functions_data, delegate directly to visualizer—do NOT re-delegate to analyst. "
            "MULTI-SUBPLOT: When user asks for 'structure functions', 'plot structure functions', 'all structure functions', 'S_p and ESS', 'all subplots', or 'structure functions page': visualizer calls plot_structure_functions in ORDER—mode=sp FIRST, then mode=ess, then mode=anomalies. "
            "CRITICAL—PLOT ORDER: When user says 'first subplot ... second subplot', delegate visualizer in THAT order: first subplot (S_p, mode=sp) FIRST, second subplot (ESS, mode=ess) SECOND. "
            "When user asks for different colors per subplot: pass the style_updates that apply to each subplot to its corresponding call. Include full per_sim_style_structure (e.g. 256: #800080, 512: #FFFF00 for purple/yellow) in the task—do not truncate. "
            "SINGLE-SUBPLOT: When user asks for ONLY 'S_p', 'ESS', or 'anomalies' (e.g. 'plot ESS only'): ONE call with that mode."
        ),
    },
