...
Per-subplot style isolation (matches page "Select plot to configure"):
  Each tool uses its own style from real_iso_plot_styles — never inherits from other subplots.
  Subplots: Energy Fractions (A), Lumley Triangle (B), Diagonal b_ii (C), Cross-correlations (D),
  Deviations (E), Convergence (F). style_updates and axis_labels apply only to the subplot produced.
"""
...
from .._shared import get_from_cache, resolve_data_dir_and_find_files, save_to_cache, update_data_directory_in_context
...
def _write_back_analysis_controls(
    session_context: Dict[str, Any],
    tol_a: Optional[List[float]] = None,
    tol_c: Optional[List[float]] = None,
    tol_d: Optional[List[float]] = None,
    tol_e: Optional[List[float]] = None,
    normalize_x: Optional[bool] = None,
    x_norm: Optional[float] = None,
    stationary_iter: Optional[float] = None,
    ma_win: Optional[int] = None,
) -> None:
    if not session_context:
        return
    if tol_a is not None:
        session_context["real_iso_tol_a"] = tol_a
    if tol_c is not None:
        session_context["real_iso_tol_c"] = tol_c
    if tol_d is not None:
        session_context["real_iso_tol_d"] = tol_d
    if tol_e is not None:
        session_context["real_iso_tol_e"] = tol_e
    if normalize_x is not None:
        session_context["real_iso_norm_x"] = normalize_x
    if x_norm is not None:
        session_context["real_iso_x_norm"] = x_norm
    if stationary_iter is not None:
        session_context["real_iso_stationary_iter"] = stationary_iter
    if ma_win is not None:
        session_context["real_iso_ma_win"] = ma_win
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "style_updates": {
                        "type": "object",
                        "description": "Full Plot Style API (same as plot_spectrum): font_family, font_size, title_size, legend_size, tick_font_size, axis_title_size, font_color. Backgrounds: plot_bgcolor, paper_bgcolor. Ticks: tick_len, tick_w, ticks_outside, tick_color. Axis: x_axis_type, y_axis_type, x_tick_format, y_tick_format. Borders: show_axis_lines, axis_line_width, axis_line_color, mirror_axes. Grid: show_grid, grid_on_x, grid_on_y, grid_w, grid_dash, grid_color, grid_opacity. Curves: line_width, marker_size. Colors: palette, custom_colors. Theme: template. Legend: show_legend. Legend position: legend_x, legend_y, legend_xanchor, legend_yanchor (x,y: 0–1 inside, >1 outside; xanchor: left/center/auto; yanchor: top/middle/bottom/auto; e.g. bottom-left inside: legend_x=0.02, legend_y=0.02, legend_xanchor=left, legend_yanchor=bottom). Title: show_plot_title, plot_title. Limits: enable_x_limits, x_min, x_max, enable_y_limits, y_min, y_max. Size: enable_custom_size, figure_width, figure_height. Margins: margin_left, margin_top, margin_right, margin_bottom.",
                    },
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "style_updates": {
                        "type": "object",
                        "description": "Full Plot Style API. Legend position: legend_x, legend_y, legend_xanchor, legend_yanchor. For custom colors: palette or custom_colors. Per-curve: enable_per_curve_style, per_curve_style_Diagonal_b_ii_C: {b11: {enabled:true, color, width, dash}, b22: {...}, b33: {...}}. Axis scale always linear for b_ii.",
                    },
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "style_updates": {
                        "type": "object",
                        "description": "Full Plot Style API. Legend position: legend_x, legend_y, legend_xanchor, legend_yanchor. Per-curve: enable_per_curve_style, per_curve_style_Cross_correlations_D: {b12, b13, b23, anis: {enabled, color, width, dash}}.",
                    },
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "style_updates": {
                        "type": "object",
                        "description": "Full Plot Style API. Legend position: legend_x, legend_y, legend_xanchor, legend_yanchor. Per-curve: enable_per_curve_style, per_curve_style_Deviations_E: {devx, devy, devz, maxdev: {enabled, color, width, dash}}.",
                    },
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "style_updates": {
                        "type": "object",
                        "description": "Full Plot Style API: font_family, font_size, line_width, plot_bgcolor, grid, limits, figure_width, figure_height. Legend position: legend_x, legend_y, legend_xanchor, legend_yanchor.",
                    },
...
def execute_tool(
    ...
    if name == "plot_real_isotropy":
        ...
        normalize_x = args.get("normalize_x")
        if normalize_x is None:
            normalize_x = sess.get("real_iso_norm_x", True)
        x_norm = args.get("x_norm") if args.get("x_norm") is not None else sess.get("real_iso_x_norm")
        stationary_iter = args.get("stationary_iter") if args.get("stationary_iter") is not None else sess.get("real_iso_stationary_iter")
        ...
        plot_styles_refs = session_context.get("real_iso_plot_styles") or {}
        style_config = plot_styles_refs.get("Energy Fractions (A)")
        if style_config is None:
            style_config = default_plot_style()
            style_config.update({"x_axis_type": "linear", "y_axis_type": "linear", "line_width": 2.2})
            plot_styles_refs["Energy Fractions (A)"] = style_config
            session_context["real_iso_plot_styles"] = plot_styles_refs
        if style_updates:
        ...
        t0_raw = float(turb["iter"][0]) if turb["iter"][0] != 0 else 1.0
        x_norm_val = float(x_norm) if x_norm is not None else t0_raw
        _write_back_analysis_controls(
            session_context,
            tol_a=tol_list,
            normalize_x=normalize_x,
            x_norm=x_norm_val,
            stationary_iter=float(stationary_iter) if stationary_iter is not None else None,
            ma_win=int(ma_win) if ma_win is not None else None,
        )
    ...
    if name == "plot_lumley_triangle":
        ...
        style_updates = args.get("style_updates") or {}
        plot_styles_refs = session_context.get("real_iso_plot_styles") or {}
        style_config = plot_styles_refs.get("Lumley Triangle (B)")
        if style_config is None:
            style_config = default_plot_style()
            style_config.update({"x_axis_type": "linear", "y_axis_type": "linear", "line_width": 1.5})
            plot_styles_refs["Lumley Triangle (B)"] = style_config
            session_context["real_iso_plot_styles"] = plot_styles_refs
        if style_updates:
            style_config.update(style_updates)
            if "custom_colors" in style_updates:
    ...
    if name == "plot_diagonal_bii":
        ...
        plot_styles_refs = session_context.get("real_iso_plot_styles") or {}
        style_config = plot_styles_refs.get("Diagonal b_ii (C)")
        if style_config is None:
            style_config = default_plot_style()
            style_config.update({"x_axis_type": "linear", "y_axis_type": "linear", "line_width": 1.6})
            plot_styles_refs["Diagonal b_ii (C)"] = style_config
            session_context["real_iso_plot_styles"] = plot_styles_refs
        if style_updates:
            style_config.update(style_updates)
            if "custom_colors" in style_updates:
        ...
        tol_list = args.get("tol_list")
        if tol_list is None:
            tol_list = sess.get("real_iso_tol_c")
        if tol_list is not None and not isinstance(tol_list, list):
            tol_list = None
        t0_raw = float(turb["iter"][0]) if turb["iter"][0] != 0 else 1.0
        x_norm_val = float(x_norm) if x_norm is not None else t0_raw
    ...
    if name == "plot_cross_correlations":
        ...
        if palette_arg:
            ...
            style_updates["palette"] = palette_arg
        plot_styles_refs = session_context.get("real_iso_plot_styles") or {}
        style_config = plot_styles_refs.get("Cross-correlations (D)")
        if style_config is None:
            style_config = default_plot_style()
            style_config.update({"x_axis_type": "linear", "y_axis_type": "log", "line_width": 2.2})
            plot_styles_refs["Cross-correlations (D)"] = style_config
            session_context["real_iso_plot_styles"] = plot_styles_refs
        if style_updates:
            style_config.update(style_updates)
            if "custom_colors" in style_updates:
        ...
        tol_list = args.get("tol_list")
        if tol_list is None:
            tol_list = sess.get("real_iso_tol_d")
        if tol_list is not None and not isinstance(tol_list, list):
            tol_list = None
        if tol_list is None:
            tol_list = [0.001, 0.01]
    ...
    if name == "plot_deviations":
        ...
        if palette_arg:
            ...
            style_updates["palette"] = palette_arg
        plot_styles_refs = session_context.get("real_iso_plot_styles") or {}
        style_config = plot_styles_refs.get("Deviations (E)")
        if style_config is None:
            style_config = default_plot_style()
            style_config.update({"x_axis_type": "linear", "y_axis_type": "log", "line_width": 2.2})
            plot_styles_refs["Deviations (E)"] = style_config
            session_context["real_iso_plot_styles"] = plot_styles_refs
        if style_updates:
            style_config.update(style_updates)
            if "custom_colors" in style_updates:
        ...
        tol_list = args.get("tol_list")
        if tol_list is None:
            tol_list = sess.get("real_iso_tol_e")
        if tol_list is not None and not isinstance(tol_list, list):
            tol_list = None
        if tol_list is None:
            tol_list = [0.01, 0.02]
    ...
    if name == "plot_convergence":
        ...
        if palette_arg:
            ...
            style_updates["palette"] = palette_arg
        plot_styles_refs = session_context.get("real_iso_plot_styles") or {}
        style_config = plot_styles_refs.get("Convergence (F)")
        if style_config is None:
            style_config = default_plot_style()
            style_config.update({"x_axis_type": "linear", "y_axis_type": "log", "line_width": 1.5})
            plot_styles_refs["Convergence (F)"] = style_config
            session_context["real_iso_plot_styles"] = plot_styles_refs
        if style_updates:
            style_config.update(style_updates)
            if "custom_colors" in style_updates:
