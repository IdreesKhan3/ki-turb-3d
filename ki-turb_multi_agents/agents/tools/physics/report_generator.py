"""Agent tools for the Report Builder page (preview/add/remove/reorder/edit/generate)."""
...
def execute_tool(
    ...
    if name == "preview_report":
        ...
        report_title = (
            args.get("title")
            or session_context.get("report_title")
            or f"Turbulence Analysis Report - {datetime.now().strftime('%Y-%m-%d')}"
        )
        author = args.get("author") if args.get("author") is not None else (session_context.get("report_author") or "")
        include_toc = args.get("include_toc") if "include_toc" in args else session_context.get("report_include_toc", True)
