...
from .._shared import (
    _examples_archive_alt,
    active_job_data_dirs,
    get_from_cache,
...
CACHE_KEY_PDFS = "current_pdf_data"
...
def _resolve_velocity_file_groups(
    ...
    if not groups and dirs_to_search:
        search = dirs_to_search[0]
    ...
    return [
        {
            "name": "compute_pdfs",
            "description": (
                "Compute PDFs from velocity fields or preprocessed analysis products. "
                "Supports all PDFs page types: velocity, vorticity, enstrophy, dissipation, "
                "joint PDFs, and R-Q. Results are cached for plot_pdf."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "pdf_type": {"type": "string", "enum": PDF_TYPES},
                    "data_dir": {"type": "string", "description": "Dataset directory (optional if manifest loaded)."},
                    "file_paths": {"type": "array", "items": {"type": "string"}},
                    "source": {
                        "type": "string",
                        "description": "velocity_field | products | processed_file (default velocity_field).",
                    },
                    "bins": {"type": "integer"},
...
def execute_tool(
    ...
    if name == "compute_pdfs":
        ...
        if args.get("file_paths"):
            ...
            if products is None:
                return "Error: No analysis products loaded. Call load_analysis_products first."
            matched = [item for item in products.pdfs if pdf_type.replace("_", "") in item.variable.replace("_", "")]
            if not matched and products.pdfs:
                matched = [products.pdfs[-1]]
            if matched:
                item = matched[-1]
                cache_payload = {
                    "pdf_type": pdf_type,
                ...
                cache_key = f"{CACHE_KEY_PDFS}_{pdf_type}"
                save_to_cache(session_context, cache_key, cache_payload)
                return {"status": "success", "message": f"Loaded {pdf_type} from analysis products.", "cache_key": cache_key}
            return "Error: Requested PDF not found in analysis products; use source=velocity_field."
        else:
            files = loader.velocity_snapshots() or _find_velocity_files(
                str(loader.base_dir()), project_root, session_context, max_files=5
            )
        if not files:
            return "Error: No velocity fields found for PDF computation."
        data_dir_path = files[0].parent
        dx, dy, dz = _get_dx_dy_dz(data_dir_path, dx_override)
        if "nu" in args and args["nu"] is not None:
            nu = float(args["nu"])
        else:
            nu = loader.viscosity() or _get_nu_from_params(data_dir_path) or 0.004
        velocity = _load_velocity(files[0], fortran_order=fortran_order)
        if velocity is None:
            return f"Error: Could not load velocity from {files[0].name}"
        cache_payload: Dict[str, Any] = {"pdf_type": pdf_type, "source": "velocity_field", "file": str(files[0])}
        if pdf_type == "velocity_components":
            u_grid, pdf_u, pdf_v, pdf_w = compute_velocity_pdf(velocity, bins=bins, normalize=normalize)
            cache_payload.update(u_grid=u_grid.tolist(), pdf_u=pdf_u.tolist(), pdf_v=pdf_v.tolist(), pdf_w=pdf_w.tolist())
        elif pdf_type == "velocity_magnitude":
            xv, pv = compute_velocity_magnitude_pdf(velocity, bins=bins, normalize=normalize)
            cache_payload.update(x=xv.tolist(), y=pv.tolist())
        elif pdf_type == "vorticity":
            xv, pv = compute_vorticity_pdf(velocity, dx=dx, dy=dy, dz=dz, bins=bins, normalize=normalize)
            cache_payload.update(x=xv.tolist(), y=pv.tolist())
        elif pdf_type == "enstrophy":
            xv, pv = compute_enstrophy_pdf(velocity, dx=dx, dy=dy, dz=dz, bins=bins, normalize=normalize)
            cache_payload.update(x=xv.tolist(), y=pv.tolist())
        elif pdf_type == "dissipation":
            xv, pv = compute_dissipation_pdf(velocity, nu=nu, dx=dx, dy=dy, dz=dz, bins=bins, normalize=normalize)
            cache_payload.update(x=xv.tolist(), y=pv.tolist())
        elif pdf_type == "joint_velocity_dissipation":
            u_c, e_c, joint = compute_velocity_dissipation_joint_pdf(velocity, nu=nu, dx=dx, dy=dy, dz=dz, bins=bins, normalize=normalize)
            cache_payload.update(x=u_c.tolist(), y=e_c.tolist(), z=joint.tolist())
        elif pdf_type == "joint_velocity_enstrophy":
            u_c, o_c, joint = compute_velocity_enstrophy_joint_pdf(velocity, dx=dx, dy=dy, dz=dz, bins=bins)
            cache_payload.update(x=u_c.tolist(), y=o_c.tolist(), z=joint.tolist())
        elif pdf_type == "joint_dissipation_enstrophy":
            e_c, o_c, joint = compute_dissipation_enstrophy_joint_pdf(velocity, nu=nu, dx=dx, dy=dy, dz=dz, bins=bins, normalize=normalize)
            cache_payload.update(x=e_c.tolist(), y=o_c.tolist(), z=joint.tolist())
        else:
            r_c, q_c, joint = compute_rq_joint_pdf(velocity, r_bins=bins, q_bins=bins, dx=dx, dy=dy, dz=dz)
            cache_payload.update(x=r_c.tolist(), y=q_c.tolist(), z=joint.tolist())
        cache_key = f"{CACHE_KEY_PDFS}_{pdf_type}"
        save_to_cache(session_context, cache_key, cache_payload)
        return {"status": "success", "message": f"Computed {pdf_type} PDF.", "cache_key": cache_key}
    ...
    if file_paths_arg and isinstance(file_paths_arg, list):
        ...
        if not base.exists():
            alt = _examples_archive_alt(project_root, base_dir)
            if alt is not None:
                base = alt
        sim_items = []
        ...
        if not file_groups:
            return (
                "Error: No velocity files (*.vti, *.h5, *.hdf5) found for the active "
                "job or selected data directory. Call load_dataset_manifest or pass "
                "data_dir to the simulation outputs the user requested."
            )
    ...
    dirs_used = []
    seen = set()
    for _sp, f in sim_items:
        if f:
            d = str(Path(f).resolve().parent) if not isinstance(f, Path) else str(Path(f).parent.resolve())
            if d not in seen:
                seen.add(d)
                dirs_used.append(d)
    dirs_used = prefer_active_job_over_examples_mix(
        dirs_used, project_root, session_context
    )
    data_dir_path = Path(files[0]).parent.resolve() if files else None
    if dirs_used:
        update_data_directory_in_context(
            session_context,
            dirs_used[0],
    ...
    # Session style: use pdfs_plot_styles per subplot (mutate in place so sync captures updates)
    from utils.plot_style import default_plot_style
    plot_name = _PDF_TYPE_TO_PLOT_NAME.get(pdf_type, "Velocity Magnitude PDF")
    plot_styles_refs = session_context.get("pdfs_plot_styles") or {}
    style_config = plot_styles_refs.get(plot_name)
    if style_config is None:
        style_config = default_plot_style()
        style_config.update({"line_width": 2.4, "per_sim_style_comparison": {}, "enable_per_sim_style": False})
        plot_styles_refs[plot_name] = style_config
        session_context["pdfs_plot_styles"] = plot_styles_refs
