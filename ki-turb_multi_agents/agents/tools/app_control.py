...
from ._session_loader import load_data_into_session, load_manifest_into_session, load_manifest_into_context
...
def get_tool_definitions() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {
            "name": "load_dataset_manifest",
            "description": "Load a simulation's post-processed dataset into the app session "
                           "from a KI-TURB manifest. Accept a manifest.json path or a job_id "
                           "(resolved to simulations/<job_id>/manifest.json). Use this after "
                           "postprocess_simulation_outputs so plots read the generated data.",
            "parameters": {
                "type": "object",
                "properties": {
                    "manifest_path": {"type": "string", "description": "Path to manifest.json"},
                    "job_id": {"type": "string", "description": "Simulation job id"},
                },
            },
        },
        {
...
def execute_tool(
    ...
    if name == "load_data":
        ...
        if success:
            # KI_TURB_LOAD_DATA_CONTEXT_V1: propagate selection immediately so
            # later roles in this same LangGraph run use exactly this dataset.
            if session_context is not None:
                selected = st.session_state.get("data_directories") or []
                if not selected and st.session_state.get("data_directory"):
                    selected = [st.session_state.get("data_directory")]
                selected = [str(p) for p in selected if p]
                if selected:
                    session_context["data_directory"] = selected[0]
                    session_context["data_directories"] = selected
                if st.session_state.get("all_loaded_files") is not None:
                    session_context["all_loaded_files"] = st.session_state.get("all_loaded_files")
            return {"status": "success", "message": msg}
        return f"Error: {msg}"

    if name == "load_dataset_manifest":
        manifest_path = str(args.get("manifest_path", "")).strip()
        job_id = str(args.get("job_id", "")).strip()
        if not manifest_path and job_id:
            from .simulation import _store
            manifest_path = str(_store.job_dir(project_root, job_id) / _store.MANIFEST_FILENAME)
        if not manifest_path:
            return "Error: provide manifest_path or job_id."

        if session_context is not None:
            success, msg = load_manifest_into_context(project_root, manifest_path, session_context)
            if success:
                try:
                    from streamlit.runtime.scriptrunner_utils.script_run_context import get_script_run_ctx
                    if get_script_run_ctx() is not None:
                        load_manifest_into_session(project_root, manifest_path, st.session_state)
                except Exception:
                    pass
                return {"status": "success", "message": msg}
            return f"Error: {msg}"

        success, msg = load_manifest_into_session(project_root, manifest_path, st.session_state)
        if success:
            return {"status": "success", "message": msg}
        return f"Error: {msg}"
