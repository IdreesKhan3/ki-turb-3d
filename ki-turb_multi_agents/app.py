        ...
        with st.expander("⚙️ Privacy Settings", expanded=False):
            ...
            if config_file.exists():
                try:
                    ...
                    # Config file unreadable (permission, deleted, etc.) — keep default True
                    pass
    ...
    if not st.session_state.data_loaded:
        ...
        <div style='background: {card_bg}; padding: 0.6rem; border-radius: 4px; border-left: 3px solid {accent_color}; margin-bottom: 0.8rem;'>
            <p style='margin: 0; font-size: 0.9rem; color: {text_color}; line-height: 1.5;'>
                <strong>KI-TURB 3D</strong> analyzes <strong>Lattice Boltzmann Method (LBM)</strong> and <strong>Navier-Stokes (NS)</strong> isotropic turbulence data. Core analysis: Energy spectra <em>E</em>(<em>k</em>), structure functions <em>S</em><sub><em>p</em></sub>(<em>r</em>), scaling exponents ξ<sub><em>p</em></sub> via <strong>Extended Self-Similarity (ESS)</strong>, isotropy validation, flatness factors, probability density functions (PDFs), time series statistics, 3D visualization, and multi-simulation comparison. Use <strong>Manual mode</strong> (navigate pages) or the <strong>seven-agent Autonomous Lab</strong> (Orchestrator, Steward, Simulation, Analyst, Visualizer, Reviewer, Engineer) for the same analyses plus solver runs and product edits.
            </p>
        </div>
        """, unsafe_allow_html=True)
        
        # Feature Cards — 4×3 grid in page order (00–12)
        st.markdown(f"""
        ...
        _card = lambda title, desc: f"""
            <div style='padding: 0.5rem; border-radius: 4px; background: {card_bg}; border-left: 3px solid {accent_color}; margin-bottom: 0.4rem;'>
                <strong style='color: {text_color}; font-size: 0.9rem; display: block; margin-bottom: 0.2rem;'>{title}</strong>
                <span style='color: {secondary_text}; font-size: 0.75rem; line-height: 1.3;'>{desc}</span>
            </div>
        """
        cards = [
            ("Autonomous Lab", "Seven-agent chat: solvers, analysis, plots, engineering"),
            ("Overview", "Simulation metadata & stats"),
            ("Theory & Equations", "D3Q19, MRT matrix, equations"),
            ("Multi-Simulation", "Compare multiple DNS/LES runs"),
            ("Isotropy Validation", "Spectral and real-space analysis"),
            ("Energy Spectra", "<em>E</em>(<em>k</em>) with Kolmogorov scaling"),
            ("Flatness Factors", "<em>F</em>(<em>r</em>) and intermittency"),
            ("Structure Functions", "<em>S</em><sub><em>p</em></sub>(<em>r</em>) with ESS and scaling"),
            ("PDFs & Statistics", "PDFs, time series, Re numbers"),
            ("Time Series", "TKE, dissipation, Re evolution"),
            ("3D Visualization", "Interactive 3D volume viewer"),
        ...
        for i, col in enumerate([col1, col2, col3, col4]):
            with col:
                for j in range(3):
                    ...
                    div = _card(title, desc)
                    if j == 2:
                        div = div.replace("margin-bottom: 0.4rem;", "")
        ...
        <div style='background: {card_bg}; padding: 0.5rem; border-radius: 4px; font-size: 0.8rem; line-height: 1.6; color: {text_color};'>
            <strong>Formats:</strong> .bin, .dat, .txt, .vti, .h5, .hdf5, .csv, simulation.input, simulation.json | 
            <strong>Processing:</strong> Time-averaging, multi-sim grouping, error bars | 
            <strong>Visualization:</strong> Plotly, 3D viewer, Light/Dark themes, export PNG/PDF/SVG | 
            <strong>Analysis:</strong> Kolmogorov scaling, ESS, anomalous scaling, isotropy, PDFs, time series
        </div>
        """, unsafe_allow_html=True)
    else:
