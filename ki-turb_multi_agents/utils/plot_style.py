"""Plot style helpers for Plotly figures (pages and agent tools).

Marker/line styles, per-sim overrides, axis scale/limits, ticks, borders, grid, sidebar UI.
"""

try:
    import streamlit as st
except ImportError:  # Headless agent/test environment
    st = None
...
def color_to_rgb(color) -> tuple:
    """
    Convert color to (r, g, b) tuple in 0-255 range.
    Handles hex, rgb(...), (r,g,b) tuple, and named colors. Returns (0, 0, 0) on failure.
    """
    if color is None:
        return (0, 0, 0)
...
def color_to_hex(color) -> str:
    """Convert color to hex (#RRGGBB). Handles hex, rgb(...), named colors. Returns "#000000" on failure."""
    if color is None:
        return "#000000"
...
# Legend position presets: (x, y, xanchor, yanchor). None = use Plotly default (top-right outside).
LEGEND_POSITION_PRESETS = {
    "Default (top-right outside)": None,
    "Top-right outside": (1.02, 1, "left", "top"),
    "Top-left outside": (-0.02, 1, "right", "top"),
    "Bottom-right outside": (1.02, 0, "left", "bottom"),
    "Bottom-left outside": (-0.02, 0, "right", "bottom"),
    "Top-right inside": (0.98, 0.98, "right", "top"),
    "Top-left inside": (0.02, 0.98, "left", "top"),
    "Bottom-right inside": (0.98, 0.02, "right", "bottom"),
    "Bottom-left inside": (0.02, 0.02, "left", "bottom"),
}


def _infer_legend_position_preset(ps):
    """Infer preset name from legend_x, legend_y, legend_xanchor, legend_yanchor (e.g. when set by agents)."""
    x, y, xa, ya = ps.get("legend_x"), ps.get("legend_y"), ps.get("legend_xanchor"), ps.get("legend_yanchor")
    if x is None and y is None and not xa and not ya:
        return "Default (top-right outside)"
    for name, preset in LEGEND_POSITION_PRESETS.items():
        if preset is None:
            continue
        px, py, pxa, pya = preset
        if (x is not None and abs(float(x) - px) > 0.01) or (y is not None and abs(float(y) - py) > 0.01):
            continue
        if (xa or pxa) and (xa or "") != (pxa or ""):
            continue
        if (ya or pya) and (ya or "") != (pya or ""):
            continue
        return name
    return "Default (top-right outside)"


def render_legend_position_ui(ps: dict, key_prefix: str = ""):
    """Render legend position selectbox. Modifies ps in place. Call from page plot style sidebars."""
    _legend_pos = ps.get("legend_position") or _infer_legend_position_preset(ps)
    _legend_pos_idx = list(LEGEND_POSITION_PRESETS.keys()).index(
        _legend_pos if _legend_pos in LEGEND_POSITION_PRESETS else "Default (top-right outside)"
    )
    ps["legend_position"] = st.selectbox(
        "Legend position",
        options=list(LEGEND_POSITION_PRESETS.keys()),
        index=_legend_pos_idx,
        help="Corner of the figure for the legend (inside or outside)",
        key=f"{key_prefix}_legend_position"
    )
    preset = LEGEND_POSITION_PRESETS.get(ps["legend_position"])
    if preset is None:
        ps["legend_x"] = None
        ps["legend_y"] = None
        ps["legend_xanchor"] = None
        ps["legend_yanchor"] = None
    else:
        ps["legend_x"], ps["legend_y"], ps["legend_xanchor"], ps["legend_yanchor"] = preset
...
def convert_superscript(text):
    """Convert ^digit sequences to Unicode superscript (e.g. 512^3 → 512³)."""
    superscript_map = {
        '0': '⁰',
        '1': '¹',
        '2': '²',
...
def resolve_line_style(sim_prefix, idx, colors, ps, style_key="per_sim_style", 
    ...
    default_dash = _scalar_val(ps.get("line_dash"), "solid") or "solid"  # Agent-controllable: line_dash can be set via style_updates
    ...
    per_sim = ps.get(style_key, {})
    s = per_sim.get(sim_prefix, {})

    # When direct match missing or inactive, try prefix match (agent uses "256", manual may have "256_structure_funcs_data1")
    if (not s or not _has_override(s)) and per_sim:
        for k, v in per_sim.items():
            if isinstance(v, dict) and _has_override(v) and (sim_prefix.startswith(k + "_") or sim_prefix == k):
                s = v
                break
    if not s or not _has_override(s):
        if include_marker:
            return default_color, default_width, default_dash, default_marker, default_msize, False
        return default_color, default_width, default_dash

    color = s.get("color") or default_color
    width = float(_scalar_val(s.get("width")) or default_width)
    dash = _scalar_val(s.get("dash") or default_dash, "solid") or "solid"
...
def _has_override(sdict):
    """Entry is active if enabled=True or has any explicit style override (agent may omit enabled)."""
    if not isinstance(sdict, dict):
        return False
    if sdict.get("enabled", False):
        return True
    if sdict.get("color"):
        return True
    if sdict.get("dash") and sdict.get("dash") != "solid":
        return True
    if sdict.get("width") is not None:
        return True
    if sdict.get("marker") and sdict.get("marker") != "circle":
        return True
    if sdict.get("msize") is not None:
        return True
    return False
...
def ensure_per_sim_defaults(ps, sim_groups, style_key="per_sim_style", include_marker=False):
    """
    Ensure per-simulation style defaults exist in ps. Copies agent style when key matches
    by prefix (e.g. agent "256" → manual "256_structure_funcs_data1").
    """
    ps.setdefault(style_key, {})
    per_sim = ps[style_key]
    for k in sim_groups.keys():
        ...
        # Agent uses "256"; manual page may have "256_structure_funcs_data1" — copy agent style
        for prefix, existing in list(per_sim.items()):
            if (k.startswith(prefix + "_") or k == prefix) and _has_override(existing):
                default_style = dict(existing)
                break
        per_sim.setdefault(k, default_style)
                ...
                if include_marker:
                    ...
                    with c2:
                        s["color"] = st.color_picker(
                            "Color",
                            value=color_to_hex(s.get("color") or "#000000"),
                            key=f"{key_prefix}_over_color_{sim_prefix}",
                            disabled=not s["enabled"]
                        )
                    if not s["enabled"]:
                        s["color"] = None  # Do not store picker default when override disabled
                    with c3:
                        s["width"] = st.slider(
                            "Width",
                            0.5, 8.0,
...
def render_axis_limits_ui(ps, key_prefix=""):
    """Render UI controls for axis limits. Modifies ps in place."""
...
def apply_axis_limits(layout_kwargs, ps):
    ...
    For log scales, converts raw values to log10 (Plotly expects log range).
    """
    import numpy as np
...
def render_figure_size_ui(ps, key_prefix=""):
    """Render UI controls for figure size. Modifies ps in place."""
...
def _scalar_val(val, default=None):
    """Return scalar; if list/tuple, return first element. Used to avoid Plotly rejecting lists."""
    if val is None:
        return default
...
def _get_palette(ps):
    """Return color palette (list of hex codes) from plot style."""
...
def _axis_title_font(ps):
    """Return font dict for axis titles (family, size, color)."""
...
def _tick_font(ps):
    """Return font dict for tick labels (family, size, color)."""
    # Get font color from plot style (defaults based on template)
    font_color = ps.get("font_color")
    if font_color is None:
        # Auto-detect from template if font_color not set
...
def _normalize_plot_name(plot_name: str) -> str:
    """Normalize plot name to key format (spaces→underscores, remove parens/dashes)."""
...
def resolve_curve_style(curve: str, idx: int, colors: list, ps: dict, plot_key: str):
    ...
    Returns (color, line_width, dash, marker, marker_size).
    """
    default_color = colors[idx % len(colors)]
...
def default_plot_style():
    """Return default plot style dict with all settings."""
    return {
        "font_family": "Arial",
        "font_size": 14,
        "title_size": 16,
        ...
        "legend_position": "Default (top-right outside)",  # Preset name; used by sidebar UI
        "legend_x": None,  # 0–1 inside, >1 outside; None = Plotly default (top-right)
        "legend_y": None,
        "legend_xanchor": None,  # "left", "center", "auto"
        "legend_yanchor": None,  # "top", "middle", "bottom", "auto"
        "show_plot_title": False,  # Do not show title by default
...
def render_axis_scale_ui(ps, key_prefix=""):
    """Render UI for axis scale type (linear/log). Modifies ps in place."""
...
def render_tick_format_ui(ps, key_prefix=""):
    """Render UI for tick number format. Modifies ps in place."""
...
def render_axis_borders_ui(ps, key_prefix=""):
    """Render UI for axis borders (box). Modifies ps in place."""
    ...
    ps["axis_line_color"] = st.color_picker(
        "Border color", 
        color_to_hex(ps.get("axis_line_color") or "#000000"), 
        disabled=not ps.get("show_axis_lines", True),
...
def render_plot_title_ui(ps, key_prefix=""):
    """Render UI for plot title. Modifies ps in place."""
...
def apply_plot_style(fig, ps):
    """
    Apply plot style to a Plotly figure: fonts, colors, backgrounds, grid,
    axis scale (linear/log), borders, ticks, tick formatting.
    """
    # Get font color from plot style (defaults based on template)
    font_color = ps.get("font_color")
    if font_color is None:
    ...
    if ps.get("legend_x") is not None:
        legend_config["x"] = float(ps["legend_x"])
    if ps.get("legend_y") is not None:
        legend_config["y"] = float(ps["legend_y"])
    if ps.get("legend_xanchor"):
        legend_config["xanchor"] = ps["legend_xanchor"]
    if ps.get("legend_yanchor"):
        legend_config["yanchor"] = ps["legend_yanchor"]
    ...
    # Note: Plotly's minor axis does not support showticklabels property
    # For log scales, minor tick labels are controlled by minorloglabels (Plotly 6.3+); default may show labels on minor ticks
    ...
    xaxis_update = dict(
        ...
        ticks=tick_dir,  # "outside" or "inside" - controls tick position
    ...
    layout_limits = apply_axis_limits({}, ps)
    if layout_limits:
        if "xaxis_range" in layout_limits:
            fig.update_xaxes(range=layout_limits["xaxis_range"])
        if "yaxis_range" in layout_limits:
            fig.update_yaxes(range=layout_limits["yaxis_range"])

    # Ensure sufficient right margin to prevent clipping (legend or plot edge).
    try:
        m = fig.layout.margin
        r = getattr(m, "r", None) if m else None
        if r is None or (isinstance(r, (int, float)) and r < 80):
            fig.update_layout(margin=dict(
                l=getattr(m, "l", 80) if m else 80,
                r=80,
                t=getattr(m, "t", 80) if m else 80,
                b=getattr(m, "b", 80) if m else 80,
            ))
    except Exception:
        pass
...
def get_tick_format(axis_format, decimals, is_normalized=False):
    """Return Plotly tickformat string. axis_format: 'auto', 'integer', or 'float'."""
...
def render_legend_axis_labels_ui(traces=None,
                                  legend_names_key="custom_plot_legend_names",
                                  axis_labels_key="custom_plot_axis_labels",
                                  trace_key_func=None,
                                  reset_callback=None,
                                  key_prefix=""):
    """Sidebar UI for legend names and axis labels. Returns (legend_names, axis_labels)."""
...
def plot_style_sidebar(sim_groups=None, style_key="per_sim_style",
                       key_prefix="", include_marker=False,
                       reset_callback=None,
                       theme_selector=None, plot_name=None):
    """Plot-style sidebar controls. Returns ps; updates st.session_state.plot_style."""
    ...
    with st.sidebar.expander("🎨 Plot Style (persistent)", expanded=False):
        ...
        ps["grid_color"] = st.color_picker(
            "Major grid color", 
            color_to_hex(ps.get("grid_color") or "#B0B0B0"),
        ...
        ps["minor_grid_color"] = st.color_picker(
            "Minor grid color", 
            color_to_hex(ps.get("minor_grid_color") or "#D0D0D0"),
